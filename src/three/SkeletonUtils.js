	...
	for (let i = 0; i < bones.length; ++i) {
		...
		if (name === options.hip) {
			...
			if (options.hipPosition !== undefined) {
				globalMatrix.elements[12] +=
					options.hipPosition.x * options.scale;
				globalMatrix.elements[13] +=
					options.hipPosition.y * options.scale;
				globalMatrix.elements[14] +=
					options.hipPosition.z * options.scale;
			}
		}
...
function retargetClip(target, source, clip, options = {}) {
	...
	if (options.trim !== undefined) {
		start = Math.round(options.trim[0] * options.fps);
			...
			Math.min(Math.round(options.trim[1] * options.fps), numFrames) -
			start;
